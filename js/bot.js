...
function Game() {
  ...
  this.createField = function () {
    ...
    this.ctx.fillRect(
      this.fillWidth,
      this.marginXY,
      this.canvasWidth - 2 * this.fillWidth,
      this.canvasHeight - this.marginXY * 2
    );
    this.ctx.fillRect(
      this.marginXY,
      3 * (this.canvasHeight / this.rowsNumber),
      this.fillWidth,
      2 * (this.canvasHeight / this.rowsNumber) - this.marginXY
    );
    this.ctx.fillRect(
      this.canvasWidth - 2 * this.fillWidth,
      3 * (this.canvasHeight / this.rowsNumber),
      this.fillWidth * 2 - this.marginXY,
      2 * (this.canvasHeight / this.rowsNumber) - this.marginXY
    );
  ...
  this.mouseMoveEvent = e => {
    ...
    for (let x = 0; x <= this.rows; x++)
      for (let y = 0; y <= this.columns; y++) {
        if (graph.has(`${x}_${y}`)) {
          if (
            y * this.scale + this.wallLineWidth / 2 <= cord_X + this.scale / 2 &&
            x * this.scale + this.wallLineWidth / 2 <= cord_Y + this.scale / 2 &&
            y * this.scale + this.wallLineWidth / 2 >= cord_X - this.scale / 2 &&
            x * this.scale + this.wallLineWidth / 2 >= cord_Y - this.scale / 2
          )
            if (
  ...
  this.clickEvent = e => {
    ...
    for (let x = 0; x <= this.rows; x++) {
      for (let y = 0; y <= this.columns; y++) {
        if (graph.has(`${x}_${y}`)) {
          if (
            x * this.scale + this.wallLineWidth / 2 <= cord_X + this.scale / 2 &&
            y * this.scale + this.wallLineWidth / 2 <= cord_Y + this.scale / 2 &&
            x * this.scale + this.wallLineWidth / 2 >= cord_X - this.scale / 2 &&
            y * this.scale + this.wallLineWidth / 2 >= cord_Y - this.scale / 2
          ) {
            if (
                  ...
                  if (!wallHit) {
                    ...
                    if (selectedPoint != false) {
                      ...
                      let stopper = setInterval(() => {
                        this.draw(stopper, pathToDraw.path);
                      }, 500);
                    } else {
                      let selectedPoint = findBlockPoint(
                        `${this.curPoint.x}_${this.curPoint.y}`,
                        `4_${ownGatePoint}`,
                        graph
                      );
  ...
  this.drawPoint = function (x, y, alpha) {
    ...
    this.ctx.arc(
      y * this.scale + this.wallLineWidth / 2 + this.marginXY / 3,
      x * this.scale + this.wallLineWidth / 2 + this.marginXY / 3,
      15,
      0,
      Math.PI * 2,
      false
    ...
    this.ctx.drawImage(
      ball,
      y * this.scale + this.wallLineWidth / 2 - 25,
      x * this.scale + this.wallLineWidth / 2 - 25
    );
  ...
  this.drawGameLine = function (x1, y1, x2, y2) {
    ...
    this.ctx.moveTo(y1 * this.scale + this.marginXY, x1 * this.scale + this.marginXY);
    this.ctx.lineTo(
      x2 * this.scale + this.scale + this.wallLineWidth / 2 + this.marginXY / 3,
      y2 * this.scale + this.wallWidth / 2 + this.marginXY / 3
    );
    this.ctx.stroke();
    this.ctx.closePath();
  };
  ...
  this.draw = function (stopper, path) {
    ...
    this.drawLine(
      this.curPoint.x,
      this.curPoint.y,
      Number(element.substring(0, 1)),
      Number(element.substring(2, element.length))
    );
